...
Ohjelma toimii samanlailla kuin b ja c ja tässäkin on kovakoodattu, `password1`.
...
Python Program to Find ASCII Value of Character, Stack overflow
...
Oma ymmärrys tästä on seuraava: Koodi miinustaa ja tarkistaa vain jos käyttäjän antamassa stringissä on enää kirjaimia jäljellä. Lopulta kun lause on tarkastettu ja sen pituus on 0 koodi on tarkastanut käyttäjän inputin onnistuneesti. Koska käyttäjä antaa suoraan tyhjän stringin, koodi on "tarkastanut" sen, koska se näkee että se on tyhjä.
...
Koodissa on jossain kohtaa sellainen virhe, että se tarkistaa vain, että onko käyttäjän antama string kokonaan oikein vaihe vaiheelta. Se tarkistaa järjestyksessä niin monta merkkiä salasanasta, kuin käyttäjän inputissa on merkkejä. Se ei tarkista, että käyttäjän input olisi yhtä suuri kuin salasana JA että kaikki käyttänä inputista olisivat oikein. 
...
Testasin tätä ja se ei onnistunut. Muistin, että koodissa oltiin erikseen laitettu alkukirjaimeksi y. Testasin jos tämän laittaisi salasanan ensimmäisen u kirjaimen tilalle ja siirtäisi y-2, eli salasana olisi `wstklnmg`. Tämäkään ei toiminut. 

Sitten vielä viimeisenä oljenkortena testasin, jos tämä y(eli w kun -2) kirjain lisättäisiin lauseen alkuun tai loppuun. 

![alt text](image-36.png)

Nyt sain oikean salasanan. Tässä tuli kyllä avuksi tuo pieni python ohjelma, jonka avulla sai nopeasti siirettyä haluamansa sanan haluamallaan määrällä. 
